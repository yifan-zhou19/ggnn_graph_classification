	...
	*/
	...
	protected Vector<Point> resample(Vector<PointData> pts, int deltaTms) {
		...
		for(int i=1; i<size-1; i++) {
			if((pts.get(i).getTimeStamp()-pts.get(i-1).getTimeStamp())<deltaTms) {
				res.remove(i);
				size--;
				i--;
			} else if((pts.get(i).getTimeStamp()-pts.get(i-1).getTimeStamp())>deltaTms) {
				double k = (pts.get(i).getTimeStamp()-pts.get(i-1).getTimeStamp())/(pts.get(i+1).getTimeStamp()-pts.get(i+1).getTimeStamp());
				double x = pts.get(i-1).getX()+(pts.get(i).getX()-pts.get(i-1).getX())*k;
				double y = pts.get(i-1).getY()+(pts.get(i).getY()-pts.get(i-1).getY())*k;
				Point p = new Point();
				p.setLocation(x, y);
				res.add(p);
			} else if((pts.get(i).getTimeStamp()-pts.get(i-1).getTimeStamp())>deltaTms) {
				res.add(pts.get(i).getPoint());
			}
		}
		return res;
	}
