...
//C++ TO JAVA CONVERTER TODO TASK: Java does not allow declaring types within methods:
	...
	//	stu2[150];
	  ...
	  for (i = 1;i <= n - 1;i++)
	  {
		for (j = i + 1;j <= n;j++)
		{
		  distance[k] = (float)Math.sqrt((stu1[i].x - stu1[j].x) * (stu1[i].x - stu1[j].x) + (stu1[i].y - stu1[j].y) * (stu1[i].y - stu1[j].y) + (stu1[i].z - stu1[j].z) * (stu1[i].z - stu1[j].z));
					  stu2[k].x1 = stu1[i].x;
					  stu2[k].y1 = stu1[i].y;
					  stu2[k].z1 = stu1[i].z;
					  stu2[k].x2 = stu1[j].x;
