...
public class GlobalMembers
{
	...
	public static void major(tangible.RefObject<String> str1, tangible.RefObject<String> str2)
	{
		int[] a = new int[N];
		int[] b = new int[N];
		int len1;
		...
		int jump;
		int i;
		len1 = str1.argValue.length();
			len2 = str2.argValue.length();
			if (len1 < len2)
			{
			temp = str1.argValue;
			t = len1;
			str1.argValue = str2.argValue;
			len1 = len2;
			str2.argValue = temp;
			len2 = t;
			}
		...
		for (i = len;i >= 0;i--)
		{
			str1.argValue.charAt(i) = str1.argValue.charAt(i - 1);
		}
		str1.argValue.charAt(0) = '0';
		for (i = len;i >= jump;i--)
		{
			str2.argValue.charAt(i) = str2.argValue.charAt(i - jump);
		}
		for (i = 0;i < jump;i++)
		{
			str2.argValue.charAt(i) = '0';
		}
		for (i = len;i >= 0;i--)
		{
			a[i] = str1.argValue.charAt(i) - '0';
			b[i] = str2.argValue.charAt(i) - '0';
		}
		for (i = len;i >= 0;i--)
		{
			if (a[i] >= b[i])
