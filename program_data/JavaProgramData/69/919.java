...
{
	public static int f(String a, String b, tangible.RefObject<String> p)
	{
		 int m;
		 int n;
		 int N = 0;
		 ...
		 for (int i = 0;i < m;i++)
		 {
			 ...
			 {
			 p.argValue.charAt(n - 1 - i) = a[m - 1 - i] + b[n - 1 - i] - '0';
			 }
			 else
			 {
				 p.argValue.charAt(n - 1 - i) = a[m - 1 - i] + b[n - 1 - i] - '0' - 10;
				 if (n - 2 - i >= n - m)
				 {
		 ...
		 else
		 {
					  if (m == n)
					  {
							  for (int i = 0;i < n;i++)
							  {
							  p.argValue.charAt(n - i) = p.argValue.charAt(n - 1 - i);
							  }
					  ...
					  else
					  {
						  for (int i = 0;i < n - m;i++)
						  {
							  if (b[n - m - 1 - i].compareTo('9') < 0)
							  {
								  b[n - m - 1 - i] = b[n - m - 1 - i].Substring(1);
								  break;
							  }
							  else
							  {
