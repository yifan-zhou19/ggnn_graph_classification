...
public class GlobalMembers
{
	public static int Main()
	{
		...
		int sign2 = 0;
		int sign3 = 0;
		num[0] = new Scanner(System.in).nextLine();
		num[1] = new Scanner(System.in).nextLine();
		lenth[0] = String.valueOf(num[0]).length();
		lenth[1] = String.valueOf(num[1]).length();
		if (lenth[0] >= lenth[1]) //???????????????????????????????????????????????
		{
			lenth[2] = lenth[0];
			for (i = 0;i < lenth[0];i++)
