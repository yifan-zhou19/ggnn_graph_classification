...
public class GlobalMembers
{
	...
	*/
	...
	tangible.RefObject<String> tempRef_number = new tangible.RefObject<String>(number);
		int decNumber = change2Dec(tempRef_number, len_number, originBase); //?originBase????number???int??decNumber
		number = tempRef_number.argValue;
	tangible.RefObject<String> tempRef_newNumber = new tangible.RefObject<String>(newNumber);
		len_newNumber = changeDec2NewBase(decNumber, newBase, tempRef_newNumber); //?int?decNumber???newBase???????newNumber????
		newNumber = tempRef_newNumber.argValue;
		newNumber = tangible.StringFunctions.changeCharacter(newNumber, len_newNumber, null);
