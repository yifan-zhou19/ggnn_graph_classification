...
//C++ TO JAVA CONVERTER TODO TASK: Pointer arithmetic is detected on this variable, so pointers on this variable are left unchanged:
	 ...
	 strLength = str.argValue.length(); //length of the original sentence
	 FindWordLength = FindWord.argValue.length(); //length of the old word
	 SwapWordLength = SwapWord.argValue.length(); //length of the new word
	 ...
	 while (*p1 != '\0')
	 {
	  ...
	  //find the old word in the sentence 
	   if (n < 0)
	   {
		...
		{
		 str.argValue.charAt(j++) = SwapWord.argValue.charAt(k); //str[j++]==str[j], j++
		}
