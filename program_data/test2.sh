...
folder=Oct-15-2018-0000029  
if [ "$1" == "" ]; then
	for n in 104 50 25 10; do
	   test $n cpp_babi_format_$folder | tee -a test2.log
	done
	for n in 50 30 10; do
	   test $n github_cpp_babi_format_$folder | tee -a test2.log
	done
elif [ "$1" == "java" ]; then
	mll_test java_babi_format_$folder $2 $(pick_model java_babi_format_$folder/$2.cpkl) | tee -a test2.log
elif [ "$1" == "cpp" ]; then
	mll_test cpp_babi_format_$folder $2 $(pick_model cpp_babi_format_$folder/$2.cpkl) | tee -a test2.log
elif [ "$1" == "biggnn" ]; then
        rm -f test2.log
        n=2
        while [ $2 -gt $n ]; do
          rm -f cpp_babi_format_$folder/cll-$2.cpkl*
          rm -f cpp_babi_format_$folder/cll-log-$2.txt
          cp cll-$n.cpkl cpp_babi_format_$folder/cll-$2.cpkl
          cp cll-$n.cpkl cpp_babi_format_$folder/cll-$2.cpkl.0
          chmod o+w cpp_babi_format_$folder/cll-$2.cpkl
	  cll_test cpp_babi_format_$folder $2 $(pick_model cpp_babi_format_$folder/cll-$2.cpkl) | tee -a test2.log
          n=$(( n * 2))
	done
elif [ "$1" == "github_java" ]; then
	mll_test github_java_babi_format_$folder $2 $(pick_model github_java_babi_format_$folder/$2.cpkl) | tee -a test2.log
