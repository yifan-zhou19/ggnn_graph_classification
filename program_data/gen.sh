...
function molecules() {
	in=$1
	out=${in/protobuf/molecules}-$2
	cp ggnn/ggnn-$2.py ggnn/ggnn.py
	out0=${in/protobuf/babi}-$2
	cp $out0/maps.*.pkl .
	chmod a+w maps.*.pkl
	mkdir -p $out
	chmod -R a+w $out
	for f in $in/*.fbs; do
	  ...
	  i=${i/.fbs/}
	  if [ ! -f $out/train/train_$i.txt -o ! -f $out/test/test_$i.txt ]; then
	    ggnn --train_graph=$out/molecules_train.json --valid_graph=$out/molecules_valid.json $in/$i.fbs $out/train_$i.json $out/test_$i.json
	  fi
	done
	docker run -v $(pwd)/$out:/e --entrypoint bash -it ggnn -c "cp /usr/local/bin/ggnn /e/ggnn.py"
	cp $0 $out
...
function generate_data() {
	...
	molecules "$cpp_folder"_protobuf_format_$tag $g | tee $0.log
	#molecules "$java_folder"_protobuf_format_$tag $g | tee $0.log
	#prepare_dataset $tag-$2 $cpp_folder
}

#generate_data Oct-10-2018 0000028 github_cpp
#generate_data Oct-10-2018 0000028 cpp
generate_data Oct-15-2018 0000030 github_cpp
generate_data Oct-15-2018 0000030 cpp
	...
	#ggnn java_protobuf_format_Oct-15-2018/89.fbs java_babi_format_Oct-15-2018-0000029/train_89.json java_babi_format_Oct-15-2018-0000029/test_89.json
